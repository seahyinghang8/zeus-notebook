...
<script lang="ts">
	...
	import { debounce } from '$lib/functions/util';
	...
	export let mainCursorPos: number = 0;
	...
	function create_editor_view(): EditorView {
		const on_change = debounce(handle_change, 100);
	...
	const domEventHandlerExtension = EditorView.domEventHandlers({
		focus: (ev) => {
			dispatch('focus', ev.relatedTarget);
		},
		blur: (ev) => {
			dispatch('blur', ev.relatedTarget);
		}
	});

	const viewUpdateExtension = EditorView.updateListener.of((update: ViewUpdate) => {
		if (update.selectionSet) {
			mainCursorPos = update.state.selection.main.head;
		}
	});
